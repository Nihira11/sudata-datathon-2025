...
```{r}
df <- read_csv("~/Desktop/sudata-datathon-2025/data/raw/dynamic_supply_chain_logistics_dataset.csv") %>%
  clean_names()
...
pred_rf <- factor(pred_rf, levels = levels(test_cls$risk_classification))
confusionMatrix(pred_rf, test_cls$risk_classification)
```

```{r}
vip(model_rf$finalModel, num_features = 15) +
  ggtitle("RF: Risk Classification\nFeature Importance") +
...
pred_rf <- predict(model_rf, newdata = test_dly_cls[, feat_cols] %>% as.data.frame())
pred_rf <- factor(pred_rf, levels = levels(test_dly_cls$delayed))
confusionMatrix(pred_rf, test_dly_cls$delayed, positive = "yes")
```

```{r}
vip(model_rf$finalModel, num_features = 15) +
  ggtitle("RF: Delay Classification\nFeature Importance") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5, face = "bold", size = 16, lineheight = 1.1)) +
  geom_bar(stat = "identity", aes(fill = Importance)) +
  scale_fill_gradient(low = "orange", high = "red") +
...
model_lm_disr <- train(
  ...
  data = train_disr %>% select(-all_of(drop_cols)),
  method = "lm",
  trControl = trainControl(method = "cv", number = 5),
  preProcess = c("center", "scale")
)

pred_lm_disr <- predict(model_lm_disr, newdata = test_disr %>% select(-all_of(drop_cols)))
...
pred_xgb_disr <- predict(model_xgb_disr, newdata = test_disr %>% select(-all_of(drop_cols)))
